...
export function Header({ stickyClass }: { stickyClass: string }) {
  const [menuOpen, setMenuOpen] = useState(false);
  const [dark, setDark] = useState(false);

  useEffect(() => {
    setDark(document.body.classList.contains("dark-skin"));
  }, []);
  ...
  const toggleTheme = () => {
    const next = !dark;
    setDark(next);
    document.body.classList.toggle("dark-skin", next);
    document.body.classList.toggle("light-skin", !next);
    window.localStorage.setItem("skin", next ? "dark" : "light");
  };

  return (
    <header className={`header ${stickyClass}`.trim()}>
      <div className="header__builder">
        <div className="row">
          ...
          <div className="col-xs-8 col-sm-8 col-md-8 col-lg-8 align-right">
            <a
              href="#theme"
              className={`switcher-btn${dark ? " active" : ""}`}
              onClick={(e) => {
                e.preventDefault();
                toggleTheme();
              }}
            ...
            <div
              className={`menu-full-overlay${menuOpen ? " is-open visible has-scroll animate-active" : ""}`}
                  ...
                  <div className="row">
                    <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                      ...
                      <div className="menu-social-links">
                        ...
                        <a href={profile.github} target="_blank" rel="noopener" title="GitHub">
